...
const toDateOnly = (d)=>{ const dt=new Date(d); return new Date(dt.getFullYear(),dt.getMonth(),dt.getDate()); };
...
export default function Home(){
  ...
  const [blockedGlobal, setBlockedGlobal] = useState([]);     // NEW
  const [guests, setGuests] = useState("");
  const [range, setRange] = useState();

  useEffect(() => { api.get("/rooms").then(({data}) => setRooms(data)); }, []);

  // NEW: fetch global blocked ranges once
  useEffect(() => {
    api.get("/rooms/blocked").then(({ data }) => {
      const ranges = (data || []).map(b => ({ from: toDateOnly(b.startDate), to: toDateOnly(b.endDate) }));
      setBlockedGlobal(ranges);
    });
  }, []);

  // keep per-room blocked for filtering results
  useEffect(() => {
    ...
    <div className="max-w-6xl mx-auto p-4 space-y-6 mb-12">
      ...
      <div className="flex flex-col md:flex-row gap-3 items-start">
        <div className="w-full md:w-[50%]">
          {/* pass global blocked ranges here */}
          <CalendarRange value={range} onChange={setRange} blockedRanges={blockedGlobal}/>
        </div>
        <div className="w-full md:w-56">
          <Select value={guests} onValueChange={setGuests}>
            <SelectTrigger className="w-full"><SelectValue placeholder="Select guests" /></SelectTrigger>
            <SelectContent>
      ...
      <div className="flex items-center justify-between">
        ...
        <span className="text-sm text-gray-600">{filteredRooms.length} of {rooms.length} shown</span>
      </div>
